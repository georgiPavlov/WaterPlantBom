...
# 🌱 Start WaterPlantApp and WaterVue Locally (Without Operator)
# This script starts only the backend and frontend components locally
...
print_header() {
    echo -e "${PURPLE}========================================${NC}"
    echo -e "${PURPLE}🌱 WaterPlantApp + WaterVue (Local)${NC}"
    echo -e "${PURPLE}========================================${NC}"
}
...
if [ ! -d "WaterPlantApp" ] || [ ! -d "WaterVue" ]; then
    print_error "Please run this script from the root directory"
    print_error "Expected directories: WaterPlantApp, WaterVue"
    exit 1
fi
...
echo ""
print_header
echo -e "${GREEN}🎉 WaterPlantApp and WaterVue Started Locally!${NC}"
echo ""
...
echo -e "  Stop services:        ${YELLOW}./stop_app_vue_local.sh${NC}"
echo -e "  View logs:           ${YELLOW}tail -f logs/*.log${NC}"
echo -e "  Check status:        ${YELLOW}ps aux | grep -E '(manage.py|vite)'${NC}"
echo ""
...
cat > check_app_vue_status.sh << 'EOF'
#!/bin/bash
echo "🌱 WaterPlantApp + WaterVue Status (Local)"
echo "=========================================="
...
chmod +x check_app_vue_status.sh

print_success "System startup complete! Use './check_app_vue_status.sh' to verify services."
